...
import { downloadVideo, validateYoutubeUrl } from "@/lib/yt-dlp";
...
export async function GET(req: Request) {
  try {
    ...
    if (format !== "mp4" && format !== "webm") {
      return NextResponse.json({ error: "Invalid format. Supported formats: mp4, webm" }, { status: 400 });
    }
      ...
      async start(controller) {
        ...
        try {
          ...
          send({
            percent: 0,
            step: "error",
            message: err.message || "Failed to download and convert video.",
